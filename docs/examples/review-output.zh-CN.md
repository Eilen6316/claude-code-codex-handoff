...
审查当前 diff，重点检查给 token refresh 增加指数退避重试后的回归风险和缺失测试。

## 实现上下文

- 实现复用了 `src/lib/retry.ts`
- 当前 diff 涉及 `src/auth/refresh.ts`、`src/api/client.ts` 和 `tests/auth/refresh.test.ts`
- 对外接口保持不变，但新增了一个 retry 策略常量
- 测试覆盖了“重试后成功”和“达到重试上限后失败”，但还没有覆盖“不可重试错误立即失败”
